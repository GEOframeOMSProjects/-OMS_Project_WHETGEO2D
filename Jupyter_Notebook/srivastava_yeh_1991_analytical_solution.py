...
Analytical solution for Richardson-Richards equation
by Srivastava and Yeh 1991
...
Reference: 
Srivastava, R., & Yeh, T. C. J. (1991). 
Analytical solutions for one‐dimensional, transient infiltration 
toward the water table in homogeneous and layered soils. 
...
def initial_condition_homogeneous_soil(Ks, alpha, theta_s, theta_r, qA, psi_bottom, IMAX, KMAX, z_element, file_name):
    """ Compute the initial condition for the case of homogeneous soil
    ...
    alpha: double
            is a soil pore-size distribution parameter representing the rate of reduction in hydraulic conductivity or moisture [1/cm]
    theta_s: float
            adimensional water content at saturation
    theta_r: float
            adimensional residual water content
    qA: float 
            dimensional initial water flux at soil surface [cm h-1]
    psi_bottom: float 
            prescribed water suction at the water table [cm]   
    IMAX: int
            number of solution points on x direction
    KMAX: int
            number of solution points on z direction
    z_element: array float
            z coordinate of elements' centroid [m]
    ...
    """
    psi_0 = np.zeros(IMAX*KMAX+1)
    psi_0[0] = -9999
    for elem in range(1,IMAX*KMAX+1):
        K_0 = qA/Ks-(qA/Ks-np.exp(alpha*0))*np.exp(-z_element[elem-1]*100*alpha)
        psi_0[elem] = (1/alpha*np.log(K_0))*1e-2

    np.savetxt(file_name, psi_0, delimiter=",")
    

    
    
def initial_condition_layered_soil(L1_star, L2_star, z_star, Ks1, Ks2, alpha1, alpha2, theta_s1, theta_r1, theta_s2, theta_r2, qA1_star, qB1_star, qA2_star, qB2_star, psi_bottom, file_name):
    """ Compute the initial condition for the case of homogeneous soil
    ...
    psi_0 = np.concatenate([[-9999.0], psi1_0/100, psi2_0/100])
